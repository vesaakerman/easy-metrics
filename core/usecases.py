...
def top_downloaded_files(limit):
    ...
    pipe = [{'$match': {'action': 'DOWNLOAD_FILE_REQUEST'}},
            {'$group': {'_id': '$file(FILE_NAME(0)', 'count': {'$sum': 1}}}, {'$sort': {'count': -1}},
            {'$limit': limit}]
...
def deposited_files_by_user(limit):
    pipe = [{'$match': {'action': 'FILE_DEPOSIT'}}, {'$group': {'_id': '$user', 'count': {'$sum': 1}}},
...
def most_downloaded_datasets(limit):
    pipe = [{'$group': {'_id': '$dataset(DATASET_ID', 'count': {'$sum': 1}}}, {'$sort': {'count': -1}},
            {'$limit': limit}]
    resultdata = logscol.aggregate(pipeline=pipe)
    datasets = {}
    datalist = []
...
def datasets_by_audience():
    pipe = [{'$group': {'_id': '$EMD:audience', 'count': {'$sum': 1}}}, {'$sort': {'count': -1}}]
    resultdata = metacol.aggregate(pipeline=pipe)
    return list(resultdata)
